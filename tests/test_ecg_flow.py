...
from services.lstm_classifier import LSTMClassifier, BiLSTMModel
...
class TestECGDataRequest:
    ...
    def test_ecg_data_request_default_sampling_rate(self):
        ...
        request = ECGDataRequest(
            type="classify", session_id="test-session", ecg_data=[0.5] * 180
        )
...
class TestAD8232Simulation:
    ...
    @staticmethod
    def simulate_ad8232_packet(
        timestamp_ms: int, adc_value: int, quality: int, battery: int
    ) -> bytes:
    ...
    def test_ad8232_packet_format(self):
        ...
        packet = self.simulate_ad8232_packet(
            timestamp_ms=1000, adc_value=2048, quality=95, battery=80
        )
...
class TestLSTMClassifier:
    ...
    @patch("services.lstm_classifier.BiLSTMModel")
...
class TestBiLSTMArchitecture:
    ...
    def test_bilstm_forward_pass(self):
        ...
        model = BiLSTMModel(
            input_size=1, hidden_size_1=128, hidden_size_2=64, num_classes=2
        )
...
class TestEndToEndFlow:
    ...
    def test_full_ecg_pipeline(self):
        ...
        classifier = LSTMClassifier()
        classification, confidence, name, time, probs = classifier.classify(
            preprocessed
        )
